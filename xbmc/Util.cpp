  ...
  std::vector<std::string>& paths)
{
  ...
  {
    vector<string> directories = StringUtils::Split(paths[i], "/");
      ...
      for (int j = 0; j < items.Size(); j++)
      {
        ...
        if (StringUtils::StartsWithNoCase(strItem, videoFileNameNoExt))
        {
          ...
          else    // not a rar/zip file
          {
            ...
            {
              if (StringUtils::EqualsNoCase(URIUtils::GetExtension(strItem),item_exts[i]))
              {
  ...
  std::vector<std::string>& itemPaths )
{
  ...
  {
    CURL strZipPath = URIUtils::CreateArchivePath("zip", CURL(strArchivePath),"");
