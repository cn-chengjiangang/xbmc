  ...
  m_hasBackground(false)
{
  m_gif = NULL;
  if (!m_dll.Load())
    CLog::Log(LOGDEBUG, "Gif::Gif(): Could not load giflib");
  m_strMimeType = "";
...
bool Gif::LoadGif(const char* file)
{
  ...
  try
  {
    ...
    if (m_gif->SColorMap)
    {
      ...
      //COLOR back = m_pGlobalPalette[m_gif->SBackGroundColor];
      m_backColor.b = 0;
      m_backColor.g = 0;
      m_backColor.r = 0;
      m_backColor.x = 0;
    ...
    case DISPOSE_BACKGROUND:
      {
        ...
        bool ret = SetFrameAreaToBack(m_pTemplate, frame);
        break;
      }
...
bool Gif::ConstructFrame(GifFrame &frame, const unsigned char* src)
{
  unsigned int offset = 0;
  for (unsigned int dest_y = frame.m_top, src_y = 0; src_y < frame.m_height; ++dest_y, ++src_y)
  {
    offset = (dest_y * m_pitch) + (frame.m_left * sizeof(COLOR));
    if (offset < (m_width * m_height) * sizeof(COLOR))
    {
      unsigned char *to = frame.m_pImage + offset;
...
bool Gif::SetFrameAreaToBack(unsigned char* dest, const GifFrame &frame)
{
  unsigned int offset = 0;
  for (unsigned int dest_y = frame.m_top, src_y = 0; src_y < frame.m_height; ++dest_y, ++src_y)
  {
